...
def initArgParser():
    ...
    parser.add_argument(
        "--dev",
        help="The serial the Meshtastic device is connected to, i.e. /dev/ttyUSB0",
        default=None,
        required=False,
    )
...
def ftp_upload(hourly = False):
    ...
    try:
        ftp_server = ftplib.FTP_TLS(ftp_credentials.__hostname__, ftp_credentials.__username__, ftp_credentials.__password__,timeout=5)
...
def statistics(hourly = False):
    ...
    try:
        ...
        if module_count['startlog'] is not None:
            diff_sec = (datetime.datetime.now() - module_count['startlog']).total_seconds()
            statistics['Device Telemetry'] = math.ceil((module_count['DeviceTelemetry'] / diff_sec) * 60 * 60)
            statistics['Environment Telemetry'] = math.ceil((module_count['EnvironmentTelemetry'] / diff_sec) * 60 * 60)
            statistics['Host Metrics'] = math.ceil((module_count['HostMetrics'] / diff_sec) * 60 * 60)
        ...
        with lock:
            ...
            cur = database.cursor()
            res = cur.execute("SELECT count(*) FROM nodes where seen > unixepoch(datetime('now', '-24 hours'));")
            node_count = res.fetchone()[0]
            res = cur.execute("SELECT count(*) FROM links where seen > unixepoch(datetime('now', '-24 hours'));")
        ...
        # Get top 10 data
        top10_packets = packets.groupby(["source", "longname"])["type"].count().nlargest(10).to_dict()
        top10_types = packets.groupby(["longname", "port_name"])["type"].count().nlargest(10, "first").to_dict()
        # Create number of nodes distribution over hours graph
        hourly_nunique = packets.groupby([packets['time'].dt.hour]).source.nunique().to_numpy()
        ...
        hourly_plot.figure.suptitle("Verteilung eindeutige Knoten über die Tageszeit im Messzeitraum")
        ...
        for node, node_packets in packets.groupby(["source", "longname"]):
            ...
            node_plot.set(title=f"{long_name} / {node_id:08X} / Mesh Last: {load:0.2f}%")
            ...
            for packet_group, packet_details in node_packets.groupby(["port_name"]):
                ...
                if time_cnt > 1:
                    packet_details["delta_t"] = packet_details["time"].diff().dt.total_seconds()
                    stat = packet_details["delta_t"].agg(["median", "count"]) # "median", "mean", "min", "max"
                    mean_str = str(datetime.timedelta(seconds=math.ceil(stat["median"])))
            ...
            for ax in node_plot.axes.flat:
                ...
                for label in labels:
                    ...
                    txt = label.get_text()
                    h = 0.76 / (len(labels)+1)
                    node_plot.figure.text(1.0, 0.97 - h - (h * y), delta_t_stats.get(txt, "N/A"))
            # Save node statistics graph
            plt.savefig(f"{os.getcwd()}/web/images/{node_id:08X}.png", dpi=100, bbox_inches="tight")
            plt.close()
    ...
    try:
        with lock:
            ...
            if all:
                ...
                res = cur.execute("select * from nodes where seen > unixepoch(datetime('now', '-24 hours'));")
            for row in res:
                nodes[f'{row[0]:08X}'] = {"short": row[1], "long": row[2], "seen": row[3]}

            if all:
                res = cur.execute("select * from links;")
            else:
                res = cur.execute("select * from links where seen > unixepoch(datetime('now', '-24 hours'));")
            for row in res:
                src = row[0]
                dst = row[1]
                snr = row[2]
        ...
        d3 = d3graph(charge=2000, slider=None, verbose=40, support="Mictronics", collision=3)
...
#    257	ATAK_FORWARDER
    ...
    # Connect to serial interface that provides the Meshtastic debug log
    try:
        interface = serial.Serial(dev, 115200, 8, 'N', 1, 15)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="meshtastic trace network",
        description="Log and visualize trace routes received in Meshtastic network.",
